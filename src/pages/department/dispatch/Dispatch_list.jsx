...
import { ChevronDown, ChevronUp, Filter, FileSpreadsheet,Eye  } from "lucide-react";
...
const Dispatch_list = () => {
  ...
  const fetchFields = async () => {
    try {
      const res = await api.get("/api/dispatch/fields/");
  ...
  const handleExport = async () => {
    ...
    try {
      const res = await api.get(`/api/dispatch/export/?${params.toString()}`, {
        responseType: "blob",
      });
            ...
            <tbody>
              {records.map((item, i) => (
                <tr key={i} className="border-t hover:bg-gray-50 transition">
                  ...
                  <td className="p-2 border text-center">{item.heat_no}</td>
                  <td className="p-2 border">{item.price}</td>
                  <td className="p-2 border text-center">{item.pices}</td>
                  <td className="p-2 border text-center">{item.invoiceno}</td>
<td className="p-2 border text-center">
  {item.addpdf ? (
    <button
      onClick={() => window.open(`${BASE_URL}${item.addpdf}`, "_blank")}
      className="text-blue-600 hover:text-blue-800 flex justify-center items-center gap-1"
      title="View PDF"
    >
