...
const Forging_Data_List = () => {
  ...
  const fetchFields = async () => {
    try {
      const res = await api.get("/api/forging/fields/");
  ...
  const handleExport = async () => {
    ...
    try {
      const res = await api.get(`/api/forging/export/?${params.toString()}`, {
        responseType: "blob",
      });
      const url = window.URL.createObjectURL(new Blob([res.data]));
      const link = document.createElement("a");
      link.href = url;
      link.setAttribute("download", "forging_data.xlsx");
...
export default Forging_Data_List;
