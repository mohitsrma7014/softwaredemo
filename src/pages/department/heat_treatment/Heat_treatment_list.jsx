...
const Heat_treatment_list = () => {
  ...
  const fetchFields = async () => {
    try {
      const res = await api.get("/api/heat_treatment/fields/");
  ...
  const handleExport = async () => {
    ...
    try {
      const res = await api.get(`/api/heat_treatment/export/?${params.toString()}`, {
        responseType: "blob",
      });
      const url = window.URL.createObjectURL(new Blob([res.data]));
      const link = document.createElement("a");
      link.href = url;
      link.setAttribute("download", "ht_data.xlsx");
          ...
          <table className="w-full border-collapse text-sm">
            <thead className="bg-gray-100 text-gray-700 sticky top-0 z-10">
              <tr>
                ...
                <th className="p-3 border">Furnace</th>
            ...
            <tbody>
              {records.map((item, i) => (
                <tr key={i} className="border-t hover:bg-gray-50 transition">
                  ...
                  <td className="p-2 border">{item.furnace}</td>
                  <td className="p-2 border">{item.process}</td>
...
export default Heat_treatment_list;
