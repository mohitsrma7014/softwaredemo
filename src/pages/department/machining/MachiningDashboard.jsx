...
const MachiningDashboard = () => {
  ...
  const fetchRecords = useCallback(async () => {
    try {
      ...
      const res = await api.get(`/api/machining/machiningListAPIView/?${params.toString()}`);
      const data = res.data;
  ...
  const kpis = useMemo(() => {
    ...
    const setupIIRecords = filteredRecords.filter(record => 
      record.setup?.toUpperCase() === 'II'
    );
    
    const totalProduction = setupIIRecords.reduce((sum, record) => 
    ...
    const totalTarget = setupIIRecords.reduce((sum, record) => sum + (record.target || 0), 0);
      ...
      <div className="bg-white border rounded-xl shadow-sm mb-6">
        ...
        >
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-gray-800 font-medium">Machining Dashboard Filters</h2>
          </div>
          {showFilters ? (
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        <div className="bg-white p-4 rounded-xl shadow-sm border">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm text-gray-600">Total Production (Setup II)</p>
        ...
        <div className="bg-white p-4 rounded-xl shadow-sm border">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm text-gray-600">Total Rejection (All Setup)</p>
...
export default MachiningDashboard;