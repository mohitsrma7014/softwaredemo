...
const MarkingDashboard = () => {
  ...
  const fetchRecords = useCallback(async () => {
    try {
      ...
      const res = await api.get(`/api/marking/MarkingListAPIView/?${params.toString()}`);
      const data = res.data;
  ...
  const shopFloorAnalysis = useMemo(() => {
    ...
    const shopFloorData = filteredRecords.reduce((acc, record) => {
      const shopFloor = record.operator || 'Unknown';
      if (!acc[shopFloor]) {
        acc[shopFloor] = {
          production: 0,
          target: 0,
  ...
  return (
    <div className="p-4 bg-gray-50 min-h-screen">
      ...
      <div className="bg-white border rounded-xl shadow-sm mb-6">
        ...
        >
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-gray-800 font-medium">Marking Dashboard Filters</h2>
          </div>
          {showFilters ? (
      ...
      <div className="mt-6 grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white p-6 rounded-xl shadow-sm border">
          <h3 className="text-lg font-semibold text-gray-800 mb-4">Operator Performance</h3>
