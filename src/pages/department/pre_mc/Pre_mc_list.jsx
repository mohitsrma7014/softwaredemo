...
const Pre_mc_list = () => {
  ...
  const fetchFields = async () => {
    try {
      const res = await api.get("/api/pre_mc/fields/");
  ...
  const handleExport = async () => {
    ...
    try {
      const res = await api.get(`/api/pre_mc/export/?${params.toString()}`, {
        responseType: "blob",
      });
      const url = window.URL.createObjectURL(new Blob([res.data]));
      const link = document.createElement("a");
      link.href = url;
      link.setAttribute("download", "Pre_mc_data.xlsx");
            ...
            <tbody>
              {records.map((item, i) => (
                <tr key={i} className="border-t hover:bg-gray-50 transition">
                  ...
                  <td className="p-2 border">{item.shop_floor}</td>
