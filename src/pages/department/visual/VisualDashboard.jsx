...
const VisualDashboard = () => {
  ...
  const fetchRecords = useCallback(async () => {
    try {
      ...
      const res = await api.get(`/api/visual/VisualListAPIView/?${params.toString()}`);
      const data = res.data;
  ...
  const kpis = useMemo(() => {
    ...
    const cnc_rust = filteredRecords.reduce((sum, record) => 
      sum + (record.cnc_rust || 0)  , 0
...
const totalTarget = filteredRecords.reduce((sum, record) => sum + (record.target || 0), 0);
  ...
  const rejectionReasons = [
    ...
    { key: 'cnc_rust', label: 'CNC Rust', category: 'cnc_rust' },
  ];
    ...
    <div className="p-4 bg-gray-50 min-h-screen">
      ...
      <div className="bg-white border rounded-xl shadow-sm mb-6">
        ...
        >
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-gray-800 font-medium">Visual Dashboard Filters</h2>
          </div>
          {showFilters ? (
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
        ...
        <div className="bg-white p-4 rounded-xl shadow-sm border border-purple-200">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm text-gray-600">CNC Rust</p>
              <p className="text-xl font-bold text-purple-600">{kpis.cnc_rust?.toLocaleString()}</p>
            </div>
            <div className="p-2 bg-purple-100 rounded-full">
              <BarChart3 className="w-5 h-5 text-purple-600" />
            </div>
